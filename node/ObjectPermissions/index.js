...
const USERNAME = "<USERNAME>"; // must be admin user
...
// press p to create project
screen.key('p', () => {
    var projectform = blessed.form({
        parent: screen,
        border: {
            type: 'line',   
        },
    ...
    projectform.on('submit', (data) => {
        appstate.realm.write(() => {
            // create a project
            let project = appstate.realm.create(ProjectSchema.name, { id: uuidv4(), owner: appstate.user.identity, timestamp: new Date(), items: [], permissions: [] });

            // set up permissions
            let user = appstate.realm.objects('__User').filtered(`id = '${appstate.user.identity}'`);
            let role = appstate.realm.create(Realm.Permissions.Role, { name: uuidv4(), members: [] });
            role['members'].push(user[0]);
            let permission = appstate.realm.create('__Permission', { canRead: true, canUpdate: true, canQuery: true, canCreate: true, canDelete: true, role: role });
            //permission['role'].push(role);

            // and it permission to the project
            project['permissions'].push(permission);

            appstate.project = project;
            statusbox.pushLine(`Adding a new project: ${project.id} - ${project.owner}`);
        });
        projectform.destroy();
        statusbox.scroll(1);
        screen.render();
    ...
    .then(user => {
        ...
        Realm.open({
            ...
            .then(realm => {
                ...
                //let role = realm.objects(Realm.Permissions.User.name).filtered()
